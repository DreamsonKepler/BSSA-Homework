...
app.post('/displayAutoSearch', (request, response) => {
    let autoSearch = request.body.gorilla
	...
	fs.readFile(__dirname + '/users.json', 'utf-8', (err, data) => { 
		...
		for (let i=0; i<userData.length; i++) {
			if (userData[i].firstname.indexOf(autoSearch) > -1){
				const matchedUser = userData[i]
				// zet matchedUser in een nieuwe array
				searchResults.push(matchedUser)
			}
